...
-- ============================================================
--  I2. PROCÉDURE DE SNAPSHOT MENSUEL POUR LE ML
-- ============================================================

DROP PROCEDURE IF EXISTS sp_generate_monthly_snapshots //
CREATE PROCEDURE sp_generate_monthly_snapshots(IN p_year_month VARCHAR(7))
BEGIN
    -- Cette procédure calcule et sauvegarde la photo (snapshot) de chaque abonné
    -- pour le mois donné (format 'YYYY-MM', ex: '2026-06')
    
    INSERT INTO fact_subscriber_monthly (
        subscriber_sk, snapshot_month, plan_sk, data_used_mb, voice_used_sec, sms_used,
        revenue_fcfa, network_incidents, failed_calls, days_since_active, churn_status
    )
    SELECT 
        s.subscriber_sk,
        p_year_month AS snapshot_month,
        MAX(u.plan_sk) AS plan_sk,
        COALESCE(SUM(CASE WHEN d.mois_annee = p_year_month THEN u.data_mb ELSE 0 END), 0) AS data_used_mb,
        COALESCE(SUM(CASE WHEN d.mois_annee = p_year_month THEN u.duration_sec ELSE 0 END), 0) AS voice_used_sec,
        COALESCE(SUM(CASE WHEN d.mois_annee = p_year_month AND u.event_type = 'SMS' THEN 1 ELSE 0 END), 0) AS sms_used,
        COALESCE(SUM(CASE WHEN d.mois_annee = p_year_month THEN u.amount_fcfa ELSE 0 END), 0) AS revenue_fcfa,
        
        -- Nombre d'incidents sur l'antenne du client ce mois-là
        (SELECT COUNT(*) FROM fact_incident i 
         JOIN dim_date d2 ON i.date_sk = d2.date_sk 
         WHERE i.tower_sk = (SELECT tower_sk FROM dim_tower dt WHERE dt.tower_id = s.tower_id LIMIT 1) 
         AND d2.mois_annee = p_year_month) AS network_incidents,
         
        COALESCE(SUM(CASE WHEN d.mois_annee = p_year_month AND u.status = 'Échec' THEN 1 ELSE 0 END), 0) AS failed_calls,
        
        -- Jours depuis dernière activité (par rapport à la fin du mois concerné, évite le Data Leakage)
        COALESCE(DATEDIFF(
            LAST_DAY(STR_TO_DATE(CONCAT(p_year_month, '-01'), '%Y-%m-%d')),
            MAX(CASE WHEN d.mois_annee <= p_year_month THEN d.date_complete ELSE NULL END)
        ), 999) AS days_since_active,
        
        -- Statut Churn : 1 si le client a résilié au cours de ce mois (ou avant)
        CASE 
            WHEN s.churn = 1 AND DATE_FORMAT(s.churn_date, '%Y-%m') <= p_year_month THEN 1
            ELSE 0
        END AS churn_status
    FROM dim_subscriber s
    LEFT JOIN fact_usage u ON s.subscriber_sk = u.subscriber_sk
    LEFT JOIN dim_date d ON u.date_sk = d.date_sk AND d.mois_annee <= p_year_month
    WHERE s.is_current = 1
    GROUP BY s.subscriber_sk, s.tower_id, s.churn, s.churn_date
    ON DUPLICATE KEY UPDATE
        plan_sk = VALUES(plan_sk),
        data_used_mb = VALUES(data_used_mb),
        voice_used_sec = VALUES(voice_used_sec),
        sms_used = VALUES(sms_used),
        revenue_fcfa = VALUES(revenue_fcfa),
        network_incidents = VALUES(network_incidents),
        failed_calls = VALUES(failed_calls),
        days_since_active = VALUES(days_since_active),
        churn_status = VALUES(churn_status);
        
    SELECT CONCAT('Snapshot mensuel généré avec succès pour ', p_year_month) AS message;
END //
